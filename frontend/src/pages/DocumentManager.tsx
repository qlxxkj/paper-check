...
import {
    getAllDocs, importFiles, importMixed, openFileDialog, backupDB,
    batchMarkSource, batchDelete, exportDataZip, openDoc, deleteDoc, onImportProgress,
} from '../api';
...
const App: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const unsub = onImportProgress((data: any) => {
            setProgress(data);
            if (data.type === 'done') {
                // 清除上一次的定时刷新
                if (progressTimer) clearTimeout(progressTimer);
                if (hideTimer) clearTimeout(hideTimer);
                // 延迟800ms再刷新，确保DB WAL写入完成
                progressTimer = setTimeout(() => {
                    loadDocs();
                    // 延迟刷新完成后 3 秒自动隐藏进度条
                    hideTimer = setTimeout(() => setProgress(null), 3000);
                }, 800);
            }
        });
    ...
    const loadDocs = async () => {
        try {
            const all: Document[] = await getAllDocs();
            const sources = all.filter((d: Document) => d.IsSource === 1);
            setAllDocs(sources);
            setCurrentPage(1);
    ...
    const handleImport = async () => {
        ...
        // 立即显示进度条
        setProgress({ type: 'start', status: '准备导入...', index: 0, total: filePaths.length });
        try {
    ...
    const handleMarkSource = async () => {
        ...
        const result = await batchMarkSource(ids);
        if (result.success) {
            await loadDocs();
            alert('入库成功');
        } else {
    ...
    const handleExport = async () => {
        ...
        try {
            const result = await exportDataZip(ids);
            if (result.success) {
    ...
    const handleEdit = async (doc: any) => {
        ...
        const result = await openDoc(doc.FilePath);
    ...
    const filteredDocs = useMemo(() => {
        return allDocs.filter(doc => {
            ...
            if (docType !== 'all') {
                ...
                if (docType === 'docx' && !isDocx) return false;
                if (docType === 'doc' && isDocx) return false;
            }
            if (startDate) {
    ...
    }, [allDocs, filterStatus, keyword, docType, startDate, endDate]);
                ...
                <div style={styles.statsAndFilter}>
                    ...
                    <div style={styles.filterBar}>
                        <div style={styles.filterLeft}>
                            ...
                            <span style={styles.filterLabel}>文档类型:</span>
                            <select value={docType} onChange={(e) => setDocType(e.target.value)} style={styles.filterInput}>
                                <option value="all">全部</option>
                                <option value="docx">.docx</option>
                                <option value="doc">.doc</option>
                            </select>

                            <span style={styles.filterLabel}>日期范围:</span>
                        ...
                        <div style={styles.filterCenter}>
                            <button
                                ...
                                disabled={deleting}
                            >
                                <i className="fas fa-trash" style={{ marginRight: 4 }}></i>  {deleting ? '删除中...' : '批量删除'}
                            </button>
                        </div>
                    </div>
                </div>
