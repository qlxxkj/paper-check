...
import { getConfig, getRepeatRelations, getDuplicateParagraphs, getParagraphContext, getDocParagraphs } from '../api';
...
const CompareView: React.FC<CompareViewProps> = ({ docId, docName, onClose }) => {
    ...
    const loadRelations = async () => {
        setLoading(true);
        try {
            const data = await getRepeatRelations(docId);
            setRelations(data);
            if (data.length > 0) {
    ...
    const loadDiff = async (targetDocId: number) => {
        setLoading(true);
        try {
            const data = await getDuplicateParagraphs(docId, targetDocId);
            setDiffResults(data || []);
    ...
    const handleShowContext = async (docId: number, paraIndex: number) => {
        try {
            ...
            const context = await getParagraphContext(docId, paraIndex, contextSize);
            console.log('[CompareView] 返回上下文:', context);
            if (context.length === 0) {
                console.warn('[CompareView] 上下文为空，请检查 Paragraph 表是否有该文档的段落数据。');
                // 尝试获取该文档的所有段落作为降级方案
                const allParas = await getDocParagraphs(docId);
