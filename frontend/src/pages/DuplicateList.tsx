...
const DuplicateList: React.FC = () => {
    ...
    const loadPending = async () => {
        ...
        try {
            ...
            for (const doc of pending) {
                try {
                    const relations = await getRepeatRelations(doc.DocID);
    ...
    useEffect(() => {
        ...
        // 保存取消函数，确保每次只注册一个监听器
        removeProgressListenerRef.current = onImportProgress(progressHandler);
        // 清理：移除监听器 + 清除定时器
        return () => {
